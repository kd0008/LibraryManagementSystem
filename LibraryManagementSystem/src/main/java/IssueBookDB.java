...
public static int save(String bookcallno, String bookname,int studentid,String studentname,String studentcontact){
	int status=0;
	try{
		Connection con=DB.getConnection();
		...
		if(status>0){
		PreparedStatement ps=con.prepareStatement("insert into issuebooks(bookcallno,bookname,studentid,studentname,studentcontact) values(?,?,?,?,?)");
		ps.setString(1,bookcallno);
