...
public class ReturnBookDB {
	public static int delete(String bookcallno,String bookname,int studentid){
	int status=0;
	try{
            Connection con=DB.getConnection();		
		status=updatebook(bookcallno);//updating quantity and issue
		if(status>0){
		PreparedStatement ps=con.prepareStatement("delete from issuebooks where bookcallno=? and bookname=? andstudentid=?");
